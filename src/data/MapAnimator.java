...
/*	public void loadMap() {
		//get map dimensions
		mapWidth = GameMap.getInstance().getGameMapSizeX();
		mapHeight = GameMap.getInstance().getGameMapSizeY();
	}
	...
	public static void drawMap () {
		...
		for (int i = 0; i < mapWidth; i++) {
			for (int j = 0; j < mapHeight; j++) {
				...
				//}
				mapPanel.add(new JLabel(new ImageIcon(GameMap.getInstance().getTile(j, i).getPath() + pixelSize + fileType)));
			}
		}
		mapPanel.pack();
